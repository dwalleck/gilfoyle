...
- Read the signed `.gilfoyle/runs/<feature-slug>/spec.json` and verify its digest before probing.
- Keep probe source under the same run directory; it may be shell, Python, or another minimal executable form.
- Return `probe-result` through `structured_output` using `.pi/schemas/probe-result.schema.json`.
- Record run ID, spec digest, probe command/result, independent oracle command/result, expected and actual outputs, agreement, learned fact, related tracker IDs, and artifact hashes.
- `CONTINUE` requires a real-codebase probe, an independent mechanism, item-by-item agreement, and at least one non-trivial learned fact.
- Disagreement returns `HALT_FALSIFIED` with implicated leg and evidence; it never advances to design.
- Missing/stale evidence, path escape, or unsigned input returns `NEEDS_DECISION`.
- Never edit production feature code, the specification, or expected oracle output to obtain agreement.
- Markdown output examples later in this file are explanatory only; this JSON contract controls Pi output.
