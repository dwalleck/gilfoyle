...
- Read the current signed specification and accepted probe result from `.gilfoyle/runs/<feature-slug>/`.
- Return the design through `structured_output` using `.pi/schemas/design.schema.json`.
- Every claim has a stable normalized key, covered input shapes, removed-invariant disposition, independent falsifier, named buggy implementation, distinct expected/actual output, cost, status, and regression fence.
- Normalize keys with Unicode case-folding; duplicate normalized keys reject the entire result.
- Every new capability has a `placements[]` entry naming its owning module, whether it opens a new seam, what the implementer is forbidden to do, and a `coverage` key pointing at the claim whose falsifier is mechanical (visibility, dependency direction, or a test in the owning crate). Extend-existing work with no new capability returns an empty array.
- Record negative space with at least three settled boundaries and verified tracker IDs for actual deferrals.
- `CONTINUE` requires the cheapest falsifier to have run and passed; a fired falsifier returns `HALT_FALSIFIED`.
- Stale run/spec digests, missing probe agreement, or ambiguous implicated legs return `NEEDS_DECISION`.
- Never write feature code or alter the probe/oracle to validate a claim.
...
Each placement decision that could silently regress becomes a claim in step 3 with a *mechanical* falsifier: visibility (`pub(crate)` makes the wrong call site a compile error), a dependency-direction check, or a test written in the owning crate — a test can only exercise code placed where the test can see it. "The reviewer will notice" is not a falsifier. In Pi output, each decision is a `placements[]` entry whose `coverage` names the claim key carrying that falsifier, the same way `removedInvariants[].coverage` does.
