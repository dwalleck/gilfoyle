...
This independent Pi scope keeps the methodology below but replaces its Markdown decision log.

- Parse at most 12 findings and return decisions through `structured_output` using the review schemas.
- Normalize stable finding keys with Unicode case-folding; malformed, duplicate, or over-limit manifests launch no validators.
- A fresh read-only validator independently reproduces each bug claim and evaluates the proposed fix.
- Each result is `accept`, `modify`, `reject`, or `duplicate`, with evidence, rationale, severity, affected paths, and verified tracker ID when work is deferred.
- Rejected/duplicate work never reaches the writer; accepted/modified work is collected in input order.
- Exactly one `gilfoyle-implementer` applies collected work, followed by one fresh gatekeeper run.
- Validators never modify source or use `subagent`; child prose is not acceptance evidence.
- Markdown examples later in this file are explanatory only; the structured decision result controls Pi routing.
