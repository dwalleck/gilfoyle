...
This independent Pi scope keeps the cycle below and makes its evidence machine-readable.

- Run `RED → GREEN → BUDGET → ORACLE → REFACTOR` inside one accepted plan slice.
- Record the observed failing test before implementation and the passing test after implementation.
- BUDGET records complexity at production scale and measured wall time when the plan declares one.
- ORACLE rebuilds the product and records independent expected/actual output plus artifact hashes.
- REFACTOR begins only after GREEN, BUDGET, and ORACLE receipts pass and remain current.
- A missing/failed receipt, stale digest, regression fence failure, or out-of-slice edit returns a halt result; never edit expected output to pass.
- Return evidence through the enclosing implementer's `structured_output` result; prose cannot close the slice.
