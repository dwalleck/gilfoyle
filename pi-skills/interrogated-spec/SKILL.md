...
This independent Pi scope keeps the methodology below but replaces its Markdown artifact contract.

- The root Pi session runs this skill interactively; it is never delegated to a leaf.
- Store authoritative data under `.gilfoyle/runs/<feature-slug>/`.
- Write `raw-request.json`, `related-issues.json`, and `spec.json`; do not require Markdown mirrors.
